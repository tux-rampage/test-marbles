import { cold, hot, marbleTest } from '../index';
import { switchAll } from 'rxjs/operators';

describe('toHaveSubscriptions matcher', () => {
  it(
    'Should figure out single subscription points',
    marbleTest(() => {
      const x = cold('--a---b---c--|');
      const xsubs = '------^-------!';
      const y = cold('---d--e---f---|');
  ...
  it(
    'Should figure out multiple subscription points',
    marbleTest(() => {
      const x = cold('--a---b---c--|');
  ...
  it(
    'Should verify that switchMap was not performed due to an error',
    marbleTest(() => {
      const x = cold('--a---b---c--|');
      const y = cold('---#-x--', { x });
